...
local function remove_excess_newlines(str)
...
M.parse_curl_args = function(provider, code_opts)
  ...
  local messages = M.parse_message(code_opts)

  --- Clean up messages by removing extra newlines
  -- Remove excess newlines from each message's content
  for _, message in ipairs(messages) do
    if type(message.content) == "string" then
      message.content = remove_excess_newlines(message.content)
    end
  end
