...
local function save_request_body_to_file(spec, file_path)
    local request_body = vim.json.encode(spec.body)
    local file = io.open(file_path, "w")

    if file then
        file:write(vim.inspect(request_body))  -- Write the JSON body to the file
        file:close()
        print("Response body saved to file: " .. file_path)
    else
...
M.stream = function(opts)
  ...
  local user_prompts = vim
    .iter({
      -----> _context.avanterules is mandatory, and put the entire file as context. Becouse of that I had to comment him. (the others were commented just to make sure.)
  ...
  active_job = curl.post(spec.url, {
    ...
    callback = function(result)
      ...
      if result.status >= 400 then
        ...
        save_request_body_to_file(spec, "/home/ph/Projects/zzzz-Prompts/avante-json.md")
      end
    end,
  })
